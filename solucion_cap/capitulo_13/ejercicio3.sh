#!/bin/bash
: '
Solucion para el problema de shar que incluye conteo de caracteres
...
- Cuenta los caracteres de cada archivo original
- Verifica que los archivos extraidos tengan el mismo conteo
- Reporta errores si hay discrepancias en el conteo
...
# Procesar argumentos y recopilar archivos
for arg in "$@"; do
  if [[ ! -e "$arg" ]]; then
...
for file in "${files[@]}"; do
  ...
  # Verificar conteo de caracteres del archivo extraido
  echo "extracted_count=\$(wc -c < '${safe_file}')"
  echo "if [ \"\$extracted_count\" -ne $char_count ]; then"
  echo "    echo \"$(basename "$file"): expected $char_count characters, extracted \$extracted_count\" >&2"
  echo "fi"
