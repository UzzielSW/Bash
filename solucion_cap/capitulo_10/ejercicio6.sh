#!/bin/bash
: '
Problema: Modificar mycp para usar sed en lugar del bucle while para procesar
los argumentos escritos en la linea de comandos.

Solucion: Usar sed para procesar la entrada de argumentos y extraer archivos
fuente y destino, eliminando la necesidad de bucles for tradicionales
'

# Variable global para controlar verificacion de sobrescritura
no_clobber_check=false

# Funcion para solicitar argumentos interactivamente
prompt_for_args() {
  local source_input dest_input

  # Solicitar archivo fuente
  printf "Nombre del archivo fuente? "
  read -r source_input
  [[ -z "$source_input" ]] && {
    echo "Error: Nombre de archivo fuente requerido" >&2
    exit 1
  }

  # Solicitar archivo destino
  printf "Nombre del archivo destino? "
  read -r dest_input
  [[ -z "$dest_input" ]] && {
    echo "Error: Nombre de archivo destino requerido" >&2
    exit 1
  }

  # Expandir patrones glob si existen
  eval "source_files=($source_input)"
  to="$dest_input"

  # Configurar arrays globales usando sed para procesar
  filelist=($(printf "%s\n" "${source_files[@]}" | sed '/^$/d'))
}

# Procesar opciones con getopts
while getopts "n" opt; do
  case $opt in
  n)
    no_clobber_check=true
    ;;
  \?)
    echo "Opcion invalida: -$OPTARG" >&2
    echo "Uso: mycp [-n] archivo1 archivo2" >&2
    echo "     mycp [-n] archivo(s) dir" >&2
    echo "     mycp (modo interactivo)" >&2
    exit 1
    ;;
  esac
done

# Ajustar indices de argumentos despues de procesar opciones
shift $((OPTIND - 1))

# Validacion inicial de argumentos y modo interactivo
if [[ $# -eq 0 ]]; then
  # Modo interactivo - solicitar argumentos
  prompt_for_args
elif [[ $# -lt 2 ]]; then
  echo "Uso: mycp [-n] archivo1 archivo2" >&2
  echo "     mycp [-n] archivo(s) dir" >&2
  echo "     mycp (modo interactivo)" >&2
  exit 1
else
  # Modo normal - usar sed para procesar argumentos de linea de comandos
  to="${@: -1}"
  # Usar sed para filtrar y procesar argumentos fuente
  mapfile -t filelist < <(printf "%s\n" "${@:1:$#-1}" | sed '/^$/d; s/[[:space:]]*$//')
fi

# Validar destino para multiples archivos
if [[ ${#filelist[@]} -gt 1 && ! -d "$to" ]]; then
  echo "Error: El destino debe ser un directorio cuando se copian multiples archivos" >&2
  exit 1
fi

# Arrays para clasificar archivos - usar sed para procesar listas
existing_files=()
missing_files=()
copylist=()

# Clasificar archivos por existencia usando sed para procesar nombres
mapfile -t existing_files < <(printf "%s\n" "${filelist[@]}" | sed -n '/^./ { p }' | while read -r file; do [[ -e "$file" ]] && echo "$file"; done)
mapfile -t missing_files < <(printf "%s\n" "${filelist[@]}" | sed -n '/^./ { p }' | while read -r file; do [[ ! -e "$file" ]] && echo "$file"; done)

# Reportar archivos faltantes usando sed para formateo
if [[ ${#missing_files[@]} -gt 0 ]]; then
  echo "Advertencia: Los siguientes archivos no existen y seran omitidos:" >&2
  printf "%s\n" "${missing_files[@]}" | sed 's/^/  /' >&2
fi

# Salir si no hay archivos validos
if [[ ${#existing_files[@]} -eq 0 ]]; then
  echo "Error: No se encontraron archivos fuente validos" >&2
  exit 1
fi

# Procesar archivos existentes usando sed para generar lista de copia
if [[ "$no_clobber_check" == true ]]; then
  # Sin verificacion - agregar todos los archivos existentes
  copylist=("${existing_files[@]}")
else
  # Procesar cada archivo para verificacion de sobrescritura
  mapfile -t copylist < <(
    printf "%s\n" "${existing_files[@]}" | sed -n '/^./ { p }' | while read -r from; do
      # Determinar archivo destino
      if [[ -d "$to" ]]; then
        tofile="$to/$(basename "$from")"
      else
        tofile="$to"
      fi

      # Verificar sobrescritura
      if [[ -e "$tofile" ]]; then
        printf "%s ya existe; sobrescribir (si/no)? " "$tofile" >&2
        read -r answer </dev/tty
        if [[ $answer =~ ^(s|si)$ ]]; then
          echo "$from"
        fi
      else
        echo "$from"
      fi
    done
  )
fi

# Ejecutar copia si hay archivos en la lista
if [[ ${#copylist[@]} -gt 0 ]]; then
  if cp "${copylist[@]}" "$to"; then
    echo "Se copiaron exitosamente ${#copylist[@]} archivo(s)"
  else
    echo "Error: La operacion de copiado fallo" >&2
    exit 1
  fi
else
  echo "No se copiaron archivos"
fi
